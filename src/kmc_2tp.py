...
def gen_MSM(alpha1,alpha2,e_a,e_b):
    ...
    # define energies of states in basin A and B
    f_basinA = np.random.normal(loc=0,scale=.01/beta,size=3) + np.ones(3)*e_a
    f_basinB = np.random.normal(loc=0,scale=.01/beta,size=3) + np.ones(3)*e_b
...
def simulate(e_a,e_b,alpha1,alpha2,beta,epochs,init_state=0):
    ...
    theor_probA = np.exp(-beta*(e_a - 1 / beta * np.log(3))) / (np.exp(-beta*(e_a - 1 / beta * np.log(3))) +np.exp(-beta*(e_b - 1 / beta * np.log(3))))
    theor_probB = np.exp(-beta*(e_b - 1 / beta * np.log(3))) / (np.exp(-beta*(e_a - 1 / beta * np.log(3))) +np.exp(-beta*(e_b - 1 / beta * np.log(3))))
    ...
    print("Average equilibrium probability of state B: ", np.round(probs_dict['prob_B'],decimals=5))
